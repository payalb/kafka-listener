...
public class KafkaConsumerService {
    ...
    @Autowired DeadLetterLogRepository deadLetterLogRepository;
    @Autowired
    ...
    public void listen() {
        while (true) {
            ...
            ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(pollDuration)); // Poll messages every 1 second
    ...
    private void retryMessage(ConsumerRecord<String, String> record) {
    	CompletableFuture.runAsync(() -> processMessage(record), service)
        .exceptionally(ex -> {
    ...
    // Send failed messages to Dead Letter Queue. Can be kept on ddl for some duration and again sent back to my-topic
    private void sendToDeadLetterQueue(ConsumerRecord<String, String> record) {
    	//Save to db
    	 DeadLetterLog logEntry = new DeadLetterLog("dead-letter-topic", record.key(), record.value(), LocalDateTime.now());
